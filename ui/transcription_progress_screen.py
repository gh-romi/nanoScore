...
class TranscriptionProgressScreen(QWidget):
    ...
    def __init__(self):
        ...
        for i in range(1, 5):
            # 1. Create a circular container
            circle_frame = QFrame()
            circle_frame.setFixedSize(80, 80)
            
            # 2. Add a layout to perfectly center everything inside the circle
            ...
            # 3. Create the Text Label ("Step 1")
            text_lbl = QLabel(f"Step {i}")
            text_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            
            # 4. Create your Custom SVG Icon!
            icon_widget = AnimatedSvgWidget("icons/Check_white.svg")
            
            icon_widget.setFixedSize(45, 45)  # icon size
            icon_widget.hide() # Keep it hidden until the step is "done"
            
            circle_layout.addWidget(text_lbl)
            circle_layout.addWidget(icon_widget)
            
            # Store all three elements so we can control them later
            self.step_circles.append((circle_frame, text_lbl, icon_widget))
            steps_layout.addWidget(circle_frame)
    ...
    def set_current_step(self, step_number):
        ...
        # 1. Styles for the outer circle
        done_style = "QFrame { background-color: #026BBC; border-radius: 40px; }"
        active_style = "QFrame { background-color: white; border: 3px solid #026BBC; border-radius: 40px; }"
        future_style = "QFrame { background-color: white; border: 1px solid #777777; border-radius: 40px; }"
        
        # 2. Styles for the text inside the circle
        text_active_style = "QLabel { color: #026BBC; font-weight: bold; font-size: 20px; background: transparent; border: none; }"
        text_future_style = "QLabel { color: #777777; font-weight: normal; font-size: 18px; background: transparent; border: none; }"
        
        # 3. Apply the styles and show/hide the correct elements
        for i, (circle, text_lbl, icon_widget) in enumerate(self.step_circles):
            idx = i + 1
            if idx < step_number:
                # Step is done: Blue circle, hide text, SHOW SVG ICON!
                circle.setStyleSheet(done_style)
                text_lbl.hide()
                icon_widget.show()
            elif idx == step_number:
                # Current step: Outlined circle, show text, hide icon
                circle.setStyleSheet(active_style)
                text_lbl.setStyleSheet(text_active_style)
                text_lbl.show()
                icon_widget.hide()
            else:
                # Future step: Grey outlined circle, show text, hide icon
                circle.setStyleSheet(future_style)
                text_lbl.setStyleSheet(text_future_style)
                text_lbl.show()
                icon_widget.hide()
